...
struct FragmentAttr {
    ...
    vec3 normal;//normal (法线) 通常表示一个表面的法向量。法向量是垂直于表面（如三角形、平面等）的向量
    vec3 pos_mv;//pos_mv (变换后的顶点位置)这个变量保存了顶点在变换（模型矩阵与视图矩阵结合）后的坐标，即经过 模型变换（Model Transformation） 和 视图变换（View Transformation） 后的顶点位置。
    FragmentAttr() {}
